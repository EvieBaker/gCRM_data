...
def plot_zplot(X):
    ...
    nat_g = 0
    if (X['sample_copy'] > 1): #only if second go 
        g_1 = input('The most recent growth rate used for sample {} was {} m/s. If you want to re-use these variables enter K, to change them enter any other charactor'.format(X['names'][0], X['growth_rate']))
    else:
        g_1 = 'L'
    if (g_1 != 'K') or (X['sample_copy'] < 2):    
        while (nat_g == 0):
            try:
                nature_growth_rate = (input("Input the natural growth rate in m/s:" )) 
